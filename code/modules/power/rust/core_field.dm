...
/obj/effect/rust_em_field/New(loc, var/obj/machinery/power/rust_core/new_owned_core)
	...
	volume_covered = PI * major_radius * minor_radius * 2.5 * 2.5 * 1000 //MAYBE VRONG FORMULA
...
/obj/effect/rust_em_field/process()
	...
	if(held_plasma.toxins < transfer_ratio * 1000)		//THIS CAN BE SLOOOOOW (and don't working)
...
/obj/effect/rust_em_field/proc/AddEnergy(var/a_energy, var/a_mega_energy, var/a_frequency)
	...
	energy += a_energy * (1 - energy_loss_ratio)
	mega_energy += a_mega_energy * (1 - energy_loss_ratio)
